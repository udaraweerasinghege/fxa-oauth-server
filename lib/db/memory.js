...
MemoryStore.prototype = {
  ...
  getRedirects: function getRedirects(_clientId) {
    var clientId = this._unbuf(_clientId);
    var ids = Object.keys(this.redirects);
    var redirects = [];
    ids.forEach(function(id) {
      if (this.redirects[id].clientId === clientId) {
        redirects.push(this.redirects[id]);
      }
    }, this);
    return P.resolve(redirects);
  },
  addRedirect: function addRedirect(uri, clientId) {
    var id = this.nextRedirectId++;
    var redirect = {
      id: id,
      uri: uri,
      clientId: this._unbuf(clientId)
    };
