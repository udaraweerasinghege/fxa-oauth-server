...
const QUERY_CLIENT_REGISTER =
  'INSERT INTO clients (id, name, imageUri, secret, whitelisted) ' +
  'VALUES (?, ?, ?, ?, ?, ?);';
...
const QUERY_REDIRECTS_INSERT = 'INSERT INTO redirects (clientId, uri) ' +
  'VALUES (?, ?)';
const QUERY_REDIRECTS_FIND = 'SELECT * FROM redirects WHERE clientId=?';
...
MysqlStore.prototype = {
  ...
  getRedirects: function getRedirects(clientId) {
    var d = P.defer();
    var id = this._buf(clientId);
    this._connection.query(QUERY_REDIRECTS_FIND, [id], function(err, rows) {
      if (err) {
        return d.reject(err);
      }
      d.resolve(rows);
  ...
  addRedirect: function addRedirect(uri, clientId) {
    var d = P.defer();
    var id = this._buf(clientId);
    this._connection.query(QUERY_REDIRECTS_INSERT, [uri, id], function(err) {
      if (err) {
        return d.reject(err);
      }
      d.resolve();
